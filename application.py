...
st.set_page_config(
    page_title="Severity Analysis of OsteoArthrosis in the Knee by 5 grades"
)

class_names = ["Healthy", "Doubtful", "Minimal", "Moderate", "Severe"]

model = tf.keras.models.load_model("XCEPTION_finetuned (1).hdf5")
target_size = (224, 224)
...
# Sidebar
with st.sidebar:
    st.subheader("Knee-OA-Analysis WebApp")
    st.caption("*** A Deep Learning Product ***")

    st.subheader(":arrow_up: Upload the image")
    uploaded_file = st.file_uploader("Choose x-ray image")


# Body
st.header("Severity Analysis of OsteoArthrosis in the Knee")
...
if uploaded_file is not None:
    with col1:
        ...
        if st.button(
            ":arrows_counterclockwise: Predict OsteoArthrosis grade in the Knee bone"
        ):
            ...
            with st.spinner("Hang On!! Working on it."):
